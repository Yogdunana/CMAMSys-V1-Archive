...
**加密示例**：
```
原始 API Key: sk-caed24c0213947838ff9c4ff4c5883f0
...
**修改**：
- 账户：`admin@cmamsys.com` → `yogdunana@yogdunana.com`
- 用户名：`admin` → `Yogdunana`
- 密码：`admin123` → `X-Duan0719`
- 添加 API Key 加密功能
...
curl -X POST http://localhost:5000/api/auth/login \
  ...
  -d '{"email":"yogdunana@yogdunana.com","password":"X-Duan0719"}'
```
...
**状态**：✅ 已修复并验证
**测试账号**：`yogdunana@yogdunana.com` / `X-Duan0719`
